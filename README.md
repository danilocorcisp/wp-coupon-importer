Coupon Importer
A WordPress plugin that unifies coupon/deal imports from multiple affiliate networks into WP Coupons and Deals (WPCD), replacing what used to be several separate, single-network importer plugins.
...
```
Requirements
WordPress with WP Coupons and Deals (WPCD) active (the plugin writes to its `wpcd_coupons` post type and `wpcd_coupon_vendor` taxonomy)
PHP 7.4+
Notes
